 ...
 * matched against — the folder names under nice-icons' `src/source/` (each holds
...
/** Absolute path to nice-icons' `src/source` (the buildable icon tree). */
function iconSourceDir() {
  const base = readRegistry().basePath.replace('~', os.homedir());
  return path.join(base, 'icons', 'src', 'source');
}

/**
 * The buildable icon names — every folder under nice-icons' `src/source`.
 *
